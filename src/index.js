...
const ZERO_G_THRESH = 0.80; // meters per second squared
...
const detectThrowEnd = () => {
  if (d3.mean(acc_buffer.slice(-10, -5)) < ZERO_G_THRESH
  ...
  navigator.permissions.query({name:'accelerometer'}).then(function(result) {
    if (result.state === 'granted') {
      ...
      if (!acl.activated) {
        ...
        document.querySelector('.subtext').innerText = 'Your device doesn\'t support an accelerometer. Try viewing this on your phone instead?';
      }
      acl.addEventListener('activate', () => {
        document.querySelector('.subtext').innerText = 'Gimme a toss. You earned it!';
      })
